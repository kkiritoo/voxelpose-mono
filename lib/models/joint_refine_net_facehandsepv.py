...
class JointRefineNetFaceHandSepV(nn.Module):
    def __init__(self, cfg):
        super(JointRefineNetFaceHandSepV, self).__init__()
        ...
        # 
        self.grid_size = [cfg.JRN_SPACE,cfg.JRN_SPACE,cfg.JRN_SPACE]

        # st()
        self.cube_size = [cfg.JRN_GRID,cfg.JRN_GRID,cfg.JRN_GRID]
        


        

        self.v2v_channel_num_face = 68 ### face joints num
        self.v2v_channel_num_hand = 21 ### face joints num
        ...
        self.v2v_net_face = V2VNetLite(self.v2v_channel_num_face, self.v2v_channel_num_face)
        
        ### 左右手利用对称性过同一个网络，直接在batchsize那个维度cat就行了
        self.v2v_net_lhand = V2VNetLite(self.v2v_channel_num_hand, self.v2v_channel_num_hand)
        self.v2v_net_rhand = V2VNetLite(self.v2v_channel_num_hand, self.v2v_channel_num_hand)

        ### 68 + 42 = 110
        self.num_joints_wholebody = self.v2v_channel_num_face + 2 * self.v2v_channel_num_hand
    ...
    def forward(self, all_heatmaps_whole, meta, grid_centers, all_cloud=None):
        ...
        lhand_joint_index_list = [i for i in range(23+68,23+68+21)]
        rhand_joint_index_list = [i for i in range(23+68+21,23+68+21+21)]
        ...
        # cubes_hands = torch.cat((cubes_lhand, cubes_rhand), dim=1)
        # st()
        ...
        # index2 = torch.ones(2, dtype=torch.bool)

        valid_cubes_face = self.v2v_net_face(cubes_face[index])
        valid_cubes_lhand = self.v2v_net_lhand(cubes_lhand[index])
        valid_cubes_rhand = self.v2v_net_rhand(cubes_rhand[index])
        ...
        pred_face = self.soft_argmax_layer(valid_cubes_face, grids_face)

        
        # st()
        pred_lhand = self.soft_argmax_layer(valid_cubes_lhand, grids_lhand)
        pred_rhand = self.soft_argmax_layer(valid_cubes_rhand, grids_rhand)

        # st()
        # rhand_joint_index_list
        # pred_rhand = self.soft_argmax_layer(valid_cubes[:, rhand_joint_index_list, ...], grids_rhand)
        
        pred[index] = torch.cat((pred_face, pred_lhand, pred_rhand), dim=1)
