...
class PoseResNetRandSigma(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        self.rand_sigma_cls = cfg.NETWORK.RAND_SIGMA_CLS

        super(PoseResNetRandSigma, self).__init__()
        ...
        if self.rand_sigma_cls:
            self.cls_layer = nn.Linear(2048, 1)
    ...
    def forward(self, x):
        ...
        if self.rand_sigma_cls:
            x_cls = F.adaptive_avg_pool2d(x, [1,1])
            x_cls = x_cls.view(x_cls.shape[0], x_cls.shape[1])
            x_cls = self.cls_layer(x_cls)
            x_cls = x_cls.view(x_cls.shape[0])
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    model = PoseResNetRandSigma(block_class, layers, cfg, **kwargs)
