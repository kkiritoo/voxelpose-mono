...
class JointRefineNetFaceHandC(nn.Module):
    def __init__(self, cfg):
        super(JointRefineNetFaceHandC, self).__init__()
        ...
        self.v2v_net_hand = V2VNetLite(self.v2v_channel_num_hand * 2, self.v2v_channel_num_hand * 2)
    ...
    def forward(self, all_heatmaps_whole, meta, grid_centers, all_cloud=None):
        ...
        valid_cubes_hands = self.v2v_net_hand(cubes_hands[index])
        ...
        # st()
        pred_lhand = self.soft_argmax_layer(valid_cubes_hands[:, :21, ...], grids_lhand)
        pred_rhand = self.soft_argmax_layer(valid_cubes_hands[:, 21:, ...], grids_rhand)
