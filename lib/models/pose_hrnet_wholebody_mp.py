...
class HRNetWholebodyMP(nn.Module):
    def __init__(self):
        super(HRNetWholebodyMP, self).__init__()
    ...
    def forward(self, input_image):
        ...
        # st()
        # print(f'len(returned_outputs) {len(returned_outputs)}') # returned_outputs[0]表示只有一层的feature被return即heatmap
        # print(f"len(returned_outputs[0]['heatmap']) {len(returned_outputs[0]['heatmap'])}")
        # print(f"len(pose_results) {len(pose_results)}")

        
        resized_hm_out_all = -np.ones((1, 133, 128, 240))
        resized_hm_np_vis_all = -np.ones((133, 128, 240))

        for pose_index in range(len(pose_results)):

            heatmaps = returned_outputs[0]['heatmap'][pose_index]
            # (Pdb) returned_outputs[0]['heatmap'].shape
            # (3, 133, 96, 72)
            ...
            bbox_xywh = pose_results[pose_index]['bbox_xywh']
            bbox = bbox_xywh ### xywh
            ...
            resized_hm_np_vis_all = np.maximum(resized_hm_np_vis_all, resized_hm_np_vis) 
            
            resized_hm_np = resized_hm_np[np.newaxis, :]
            resized_hm_out_all = np.maximum(resized_hm_out_all, resized_hm_np) 

        resized_hm_out_all = torch.from_numpy(resized_hm_out_all)
        ...
        if show_hm_flag:
            ...
            show_img_w_hm(resized_hm_np_vis_all, img_vis, name='warped')
            # st()

        return resized_hm_out_all
