...
class JointRefineNetFaceHand(nn.Module):
    def __init__(self, cfg):
        super(JointRefineNetFaceHand, self).__init__()
        ...
        self.v2v_net_hand = V2VNetLite(self.v2v_channel_num_hand, self.v2v_channel_num_hand)
    ...
    def forward(self, all_heatmaps_whole, meta, grid_centers, all_cloud=None):
        ...
        cubes_hands = torch.cat((cubes_lhand, cubes_rhand), dim=0)
        # st()
        ...
        valid_cubes_hands = self.v2v_net_hand(cubes_hands[index2])
        ...
        pred_lhand = self.soft_argmax_layer(valid_cubes_hands[0:1, ...], grids_lhand)
        pred_rhand = self.soft_argmax_layer(valid_cubes_hands[1:2, ...], grids_rhand)
